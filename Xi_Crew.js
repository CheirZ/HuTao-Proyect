...
mobile: MethodMobile, 
browser: opcion == '1' ? ['HuTao-Proyect', 'Edge', '2.0.0'] : methodCodeQR ? ['HuTao-Proyect', 'Edge', '2.0.0'] : ['Ubuntu', 'Edge', '110.0.1587.56'],
auth: {
creds: state.creds,
...
console.log(chalk.black(chalk.bgOrange(`👑 CÓDIGO DE VINCULACIÓN 👑`)), chalk.bold.white(chalk.white(codeBot)))
}, 2000)
}}}
}
...
const subBotDirectory = './jadibots';
if (!existsSync(subBotDirectory)) {
console.log('🚩 HuTao no tiene Sub-Bots vinculados.');
return;
}
...
const mainBotAuthFile = 'seccion-activas';
try {
const mainBot = await connectionUpdate(mainBotAuthFile);
global.conns.push(mainBot);
console.log(chalk.bold.greenBright(`🚩 Hutao-proyect conectado correctamente.`))

await connectSubBots();
} catch (error) {
console.error(chalk.bold.cyanBright(`🍭 Error al iniciar HuTao: `, error))
}
})();
