...
import { handler, participantsUpdate, groupsUpdate } from '../handler.js';
...
const sessionFolder = `./HutaoJafiBot/${id}`;
...
if (intentos >= 5) {
console.log(chalk.bold.magentaBright(`[ ✿ ] ${botId} superó los 5 intentos. Eliminando sesión. `));
try {
fs.rmSync(sessionFolder, { recursive: true, force: true });
} catch (e) {
console.error(`[ ✿ ]  Error al eliminar sesión de ${botId}:`, e);
}
...
console.log(chalk.bold.magentaBright(`[ ✿ ] ${botId} → Reintentando ${intentos + 1}/5 en ${wait / 1000}s `));
...
console.error("[ ✿ ]  Error procesando group-participants.update:", err);
}});
