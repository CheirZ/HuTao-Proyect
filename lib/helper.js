...
const API = (name, path = '/', query = {}, apikeyqueryname) => (name in globalThis.APIs ? globalThis.APIs[name] : name) + path + (query || apikeyqueryname ? '?' + new URLSearchParams(Object.entries({ ...query, ...(apikeyqueryname ? { [apikeyqueryname]: globalThis.APIKeys[name in globalThis.APIs ? globalThis.APIs[name] : name] } : {}) })) : '')
