export default {
  ...
  user.coins += recompensa
    ...
    return client.sendContextInfoIndex(m.chat, `🌱 Has reclamado tu recompensa diaria de *${recompensa.toLocaleString()} ${monedas}*! (Día *1*)\n` +
    `> Día *2* » *¥${siguiente.toLocaleString()}*` +
    (perdioRacha ? `\n> ☆ ¡Has perdido tu racha de días!` : ''), {}, m, true, {})
}
    ...
    await client.sendContextInfoIndex(m.chat, `🌱 Has reclamado tu recompensa diaria de *¥${recompensa.toLocaleString()} ${monedas}* (Día *${user.dailyStreak}*)\n` +
